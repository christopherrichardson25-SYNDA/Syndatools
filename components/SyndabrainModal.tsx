...
type Props = {
  open: boolean;
  onClose: () => void;
  pageContext?: { source?: string; section?: string; lang?: string; uid?: string; email?: string };
};

export default function SyndabrainModal({ open, onClose, pageContext }: Props) {
  ...
  return (
    <dialog ref={ref} className="s-modal" onClose={onClose} onClick={(e) => e.target === ref.current && onClose()}>
      ...
      <style jsx>{`
        ...
        .s-close { border: none; background: transparent; font-size: 18px; cursor: pointer; }
